...
**A kernel-verified O-PINI serial-composition theorem for registered pipelines** (`LeanSec/Composition/Pipeline.lean`). A `PipelineGadget` carries a gadget plus its complete well-behavedness invariant; `compose` provably **reproduces that invariant** (it is a fixpoint under composition — closure), and `compose_pini` proves the composite is PINI *without assuming* the connected-experiment factorization. So `PipelineGadget.probing` gives `probingSecureSpec` for any pipeline prefix, and it instantiates to a concrete secure multi-cycle registered pipeline. To our knowledge this is the first **kernel-checked** masking-gadget composition theorem in the transition- and glitch-robust probing model. It is distinct from its nearest neighbours: concurrent Lean-4 masking-composition work (Iskander & Kirah, 2026) is value-only (no glitches, prime-field), and glitch-model composition verifiers (maskVerif, SILVER, MATCHI) are standalone tools whose soundness is a paper proof over unverified implementation code rather than a kernel-checked theorem. We'd be glad to be pointed to prior work we've missed. Verified: `leanchecker`-clean, axioms `{propext, Classical.choice, Quot.sound}`, 0 `sorry`.
