#!/bin/bash
# MATCHI transition-model check (L2 composition gate) for the masked carry-save
...
# Usage: run_mul256.sh <TOPMODULE> [RECOMB]   (TOP = mul256 | mul256_pini |
#        mul256_rndreuse | mul256_recomb | any mulN_* from gen_mul256.py N)
set -u
export PATH=.:$PATH
MATCHI=.
SYNTH=.
...
TB="tb_${TOP%%_*}.v"          # mul256_rndreuse -> tb_mul256.v
W=work_${TOP}
