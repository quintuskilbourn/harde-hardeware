// NEGATIVE CONTROL (must FAIL) — planted in the NEW composition glue: the
...
module dexloop16_sharedbus (clk, rst, tx_go, we, waddr, raddr, wdata, x, mo,
...
// sharedbus control: hold slot0 on the read port two cycles, slot1 next
wire [1:0] st_raddr = (pcnt == 3'd1 || pcnt == 3'd2) ? 2'd0 :
                           (pcnt == 3'd3) ? 2'd1 : raddr;
...
// BUG UNDER TEST (sharedbus): ONE time-multiplexed bus register carries
...
wire go = (pcnt == 3'd6);   // internal swap-go strobe
