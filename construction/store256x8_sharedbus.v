...
module store256x8_sharedbus (clk, rst, we, waddr, wdata, raddr, rdata,
                  pg_go, pg_from, pg_to);
...
reg [255:0] bus;                    // the single shared lane
reg [255:0] pg0_hold;
always @(posedge clk) begin
    if (rst) begin pg_rd0 <= 1'b0; pg_rd1 <= 1'b0; pg_wr0 <= 1'b0; pg_wr1 <= 1'b0; end
        ...
        if (pg_rd0)      bus <= ((pg_src == 3'd0) ? m0_0 : ((pg_src == 3'd1) ? m0_1 : ((pg_src == 3'd2) ? m0_2 : ((pg_src == 3'd3) ? m0_3 : ((pg_src == 3'd4) ? m0_4 : ((pg_src == 3'd5) ? m0_5 : ((pg_src == 3'd6) ? m0_6 : m0_7)))))));   // share 0 on the bus...
        else if (pg_rd1) begin
            pg0_hold <= bus;
            bus <= ((pg_src == 3'd0) ? m1_0 : ((pg_src == 3'd1) ? m1_1 : ((pg_src == 3'd2) ? m1_2 : ((pg_src == 3'd3) ? m1_3 : ((pg_src == 3'd4) ? m1_4 : ((pg_src == 3'd5) ? m1_5 : ((pg_src == 3'd6) ? m1_6 : m1_7)))))));                // ...then share 1: TRANSITION BUG
        end
    end
end
wire pg_wr = pg_wr0;
wire [255:0] pg0 = pg0_hold;
wire [255:0] pg1 = bus;
