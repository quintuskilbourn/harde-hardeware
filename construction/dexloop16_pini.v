...
module dexloop16_pini (clk, rst, tx_go, we, waddr, raddr, wdata, x, mo,
...
(* matchi_type = "sharings_dense", matchi_active = "st_out_act" *) output [31:0] rdata;
...
wire [1:0] st_raddr = (pcnt == 3'd1) ? 2'd0 :
                           (pcnt == 3'd2) ? 2'd1 : raddr;

// ---- registered per-lane read port (SLOAD); transitions stay in-lane ----
reg [15:0] rreg0, rreg1;
always @(posedge clk) begin
    rreg0 <= ((st_raddr == 2'd0) ? m0_0 : ((st_raddr == 2'd1) ? m0_1 : ((st_raddr == 2'd2) ? m0_2 : m0_3)));
    rreg1 <= ((st_raddr == 2'd0) ? m1_0 : ((st_raddr == 2'd1) ? m1_1 : ((st_raddr == 2'd2) ? m1_2 : m1_3)));
end
...
reg [15:0] rri0, rri1, rro0, rro1;
// both lanes captured at the SAME edge (the store16x4 paging rule)
always @(posedge clk) begin
    if (rst) begin
...
wire go = (pcnt == 3'd4);   // internal swap-go strobe
...
wire wb1 = (cnt == 12'd991);
wire wb2 = (cnt == 12'd992);
wire eff_we = we || wb1 || wb2;
wire [1:0] eff_waddr = wb1 ? 2'd0 : (wb2 ? 2'd1 : waddr);
// per-lane write-value mux: lane k mixes only share-k values (public selects)
wire [15:0] wv0 = wb1 ? D0 : (wb2 ? nror0 : wd0);
wire [15:0] wv1 = wb1 ? D1 : (wb2 ? nror1 : wd1);

// ---- lane 0 writes ----
always @(posedge clk) begin
    if (eff_we && eff_waddr == 2'd0) m0_0 <= wv0;
    if (eff_we && eff_waddr == 2'd1) m0_1 <= wv0;
    if (eff_we && eff_waddr == 2'd2) m0_2 <= wv0;
    if (eff_we && eff_waddr == 2'd3) m0_3 <= wv0;
end
// ---- lane 1 writes ----
always @(posedge clk) begin
    if (eff_we && eff_waddr == 2'd0) m1_0 <= wv1;
    if (eff_we && eff_waddr == 2'd1) m1_1 <= wv1;
    if (eff_we && eff_waddr == 2'd2) m1_2 <= wv1;
    if (eff_we && eff_waddr == 2'd3) m1_3 <= wv1;
end
