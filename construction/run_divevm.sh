#!/bin/bash
# MATCHI transition-model check (L2 composition gate) for the masked EVM
# multiplier tops. Direct mirror of ./run_iszero256.sh (the validated flow):
...
# Usage: run_divevm.sh <TOPMODULE> [RECOMB]   (TOP = divevm256 | divevm256_pini |
#        divevm256_rndreuse | divevm256_recomb | any mulN_* from gen_divevm256.py N)
...
TB="tb_${TOP%%_*}.v"          # divevm256_rndreuse -> tb_divevm256.v
W=work_${TOP}
