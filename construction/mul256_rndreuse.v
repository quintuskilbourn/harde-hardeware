// NEGATIVE CONTROL (must FAIL): identical to the target except u_pp_1
...
module mul256_rndreuse (clk, rst, go, a, b, r, s, prod);
...
MSKand_opini2_d2 u_pp_1 (  // BUG UNDER TEST: reuses u_pp_0's randomness r[0]/s[0]
    .ina({aa_d1[1], aa_d0[1]}), .inb({bb1[0], bb0[0]}),
    .rnd(r[0]), .s(s[0]), .clk(clk), .out({w_pp1[1], w_pp0[1]}));
