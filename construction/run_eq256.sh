...
# EQ (ISZERO(A^B)) tree. Direct mirror of ../add256/run_add256.sh (the validated 256-bit
...
# Usage: run_eq256.sh <TOPMODULE> [RECOMB]
set -u
export PATH=.:$PATH
MATCHI=.
SYNTH=.
...
iverilog -y "$W/hdl" -I "$W/hdl" -s tb -o "$W/sim" \
    -D DUMPFILE=\"$W/a.vcd\" $DEFS "$W/${TOP}_synth.v" tb_eq256.v > "$W/iverilog.log" 2>&1
